...
OpenAiClientSse::OpenAiClientSse(const std::string_view &apiKey)
    : m_apiKey(apiKey)
...
OpenAiClientSse::~OpenAiClientSse()
{
    if (m_sseClient)
        m_sseClient->disconnect();
}

bool OpenAiClientSse::createResponse(const std::string_view &prompt)
{
...
void OpenAiClientSse::setupSseClient()
{
