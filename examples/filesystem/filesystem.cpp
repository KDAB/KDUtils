...
void doTests()
{
    ...
    if (app->assetsDir().type() != KDUtils::StorageType::Asset)
        SPDLOG_CRITICAL("Assets Path is not of Asset type. Path: \"{}\"", app->assetsDir().path());
#endif

    // Try to find asset file
    auto textAssetDir = app->assetsDir().relativeDir("text");
    ...
    auto appDataDir = app->applicationDataDir(true);
    appDataDir.ensureExists();
    ...
    auto externalAppDataDir = app->applicationDataDir(false).relativeDir("text/test");
    externalAppDataDir.ensureExists();
