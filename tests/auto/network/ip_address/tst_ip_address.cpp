...
TEST_CASE("Constructor tests")
{
    ...
    {
        const IpAddress addr("192.168.1.1");
        CHECK(!addr.isNull());
        ...
        const IpAddress addrLiteral("192.168.1.1");
        CHECK(addrLiteral.toString() == "192.168.1.1");

        // Test with std::string
        const std::string ipString = "192.168.1.1";
        const IpAddress addrString(ipString);
        CHECK(addrString.toString() == "192.168.1.1");

        // Test with std::string_view
        const std::string_view ipStringView = "192.168.1.1";
        const IpAddress addrStringView(ipStringView);
        CHECK(addrStringView.toString() == "192.168.1.1");
    }

    SUBCASE("String constructor - IPv6")
    {
        const IpAddress addr("2001:0db8:85a3:0000:0000:8a2e:0370:7334");
        CHECK(!addr.isNull());
    ...
    SUBCASE("uint32_t constructor - IPv4")
    {
        ...
        const IpAddress addr(0xC0A80101);
        CHECK(!addr.isNull());
        CHECK(addr.type() == IpAddress::Type::IPv4);
        CHECK(addr.toString() == "192.168.1.1");
    }
    ...
    SUBCASE("sockaddr constructor - IPv4")
    {
        ...
        const IpAddress addr(reinterpret_cast<struct sockaddr *>(&addr4), sizeof(addr4));
        CHECK(!addr.isNull());
        CHECK(addr.type() == IpAddress::Type::IPv4);
        CHECK(addr.toString() == "192.168.1.1");
    }
    ...
    SUBCASE("sockaddr constructor - IPv6")
    {
        ...
        const IpAddress addr(reinterpret_cast<struct sockaddr *>(&addr6), sizeof(addr6));
        CHECK(!addr.isNull());
        CHECK(addr.type() == IpAddress::Type::IPv6);
    }
}
...
TEST_CASE("IPv4 special address tests")
{
    ...
    {
        const IpAddress loopback("127.0.0.1");
        ...
        const IpAddress staticLoopback = IpAddress::loopback();
    ...
    SUBCASE("Broadcast address")
    {
        ...
        // Check static constructor
        const IpAddress staticBroadcast = IpAddress::broadcast();
    ...
    SUBCASE("Any address")
    {
        ...
        // Check static constructor
        const IpAddress staticAny = IpAddress::any();
    ...
    {
        const IpAddress linkLocal("169.254.1.1");
    ...
    {
        const IpAddress multicast("224.0.0.1");
    ...
    {
        const IpAddress private1("10.0.0.1");
        const IpAddress private2("172.16.0.1");
        const IpAddress private3("192.168.1.1");
        const IpAddress public1("8.8.8.8");
...
TEST_CASE("IPv6 special address tests")
{
    SUBCASE("Loopback address")
    {
        const IpAddress loopback("::1");
        CHECK(loopback.isLoopback());
        CHECK(!loopback.isBroadcast()); // IPv6 has no broadcast
        CHECK(!loopback.isLinkLocal());

        // Check static constructor
        const IpAddress staticLoopback = IpAddress::loopback(IpAddress::Type::IPv6);
        CHECK(staticLoopback.isLoopback());
    ...
    SUBCASE("Any address")
    {
        ...
        // Check static constructor
        const IpAddress staticAny = IpAddress::any(IpAddress::Type::IPv6);
        CHECK(staticAny.type() == IpAddress::Type::IPv6);
    }

    SUBCASE("Link local address")
    {
        const IpAddress linkLocal("fe80::1");
        CHECK(linkLocal.isLinkLocal());
        CHECK(!linkLocal.isLoopback());
    }
    ...
    SUBCASE("Multicast address")
    {
        const IpAddress multicast("ff02::1");
        CHECK(multicast.isMulticast());
        CHECK(!multicast.isLoopback());
    }
    ...
    SUBCASE("Private address")
    {
        const IpAddress private1("fc00::1");
        const IpAddress public1("2001:0db8::1");
...
TEST_CASE("Subnet tests")
{
    ...
    SUBCASE("IPv6 subnet")
    {
        const IpAddress ip("2001:0db8:0000:0000:0000:0000:0000:0100");
        const IpAddress subnet("2001:0db8::");

        CHECK(ip.isWithinSubnet(subnet, 64));
        CHECK(!ip.isWithinSubnet(subnet, 120));

        const IpAddress ip2("2001:0db9::");
...
TEST_CASE("Conversion methods")
{
    ...
    SUBCASE("toIPv6")
    {
        ...
        const std::array<std::uint8_t, 16> ipv6Data = {
            0x20, 0x01, 0x0d, 0xb8,
            0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x01
        };
        const IpAddress ip(ipv6Data);

        auto result = ip.toIPv6();
        CHECK(result == ipv6Data);

        // IPv4 address returns zero array
        const IpAddress ip2("192.168.1.1");
    ...
    {
        const IpAddress ip("2001:0db8::1");
        struct sockaddr_storage storage;
        socklen_t len = sizeof(storage);
        ...
        // Check port
        auto *addr = reinterpret_cast<struct sockaddr_in6 *>(&storage);
